...
def reg_dat(regs):
    ...
    for reg in regs: 
        ...
        for k, v in data:
            try:
                if k: 
                    line_data.append(str(k).ljust(v)[0:v].encode('ascii'))
                else: 
                    line_data.append("".ljust(v))
            except UnicodeError:
...
def parse_axtime(event, session, results_file): 
    ...
    for line in reader: 
        ...
        try: 
            ...
            validate_error['results_file'] = "%s %s has a duplicate class/number: %d %s"%(reg.first_name, reg.last_name, reg.number, reg.race_class)
            return validate_error
        try: 
            reg.number = int(data['Num']) 
        except Exception: 
